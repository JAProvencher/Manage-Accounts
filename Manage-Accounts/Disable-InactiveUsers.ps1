...
function LogFileCheck {
	...
	if (!(Test-Path $LogDir -erroraction "SilentlyContinue")) {
		# Check if log file directory exists - if not, create and then create log file for this script.
		mkdir $LogDir
		New-Item "$LogPath" -type file
	}
	if (((gci $LogPath).length / 1MB) -gt $MaxLogFileSizeMB) {
		# Check size of log file - to stop unweildy size, archive existing file if over limit and create fresh. 
...
function Get-ADUserLastLogon {
	...
	$llo = $llo.tofiletime(), $user.lastlogontimestamp, $wc.tofiletime(), $user."msDS-LastSuccessfulInteractiveLogonTime" | Sort-Object -descending | Select-Object -first 1
...
$tocheck = get-aduser -filter { enabled -eq $true -and name -notlike "*$" } -properties * | ? { $_.lastlogondate -lt (get-date).adddays(-23) -and $_.distinguishedname -notin @((get-adgroupmember -identity exc.logon).distinguishedname) }
...
Foreach ($check in $tocheck) {
	if ($check.canonicalname.split('.')[0] -eq "nwpt") {
		$note = @"
...
"@
	...
	switch ($action) {
		# Determine course of action based on value of variable
...
"@
		...
		"Disable" {
			if (is-admin $check) {
				#Is the account protected by AdminSDHolder?  If so, notify AMT
				$notify.To = $AcctMgmt
				$body = @"
